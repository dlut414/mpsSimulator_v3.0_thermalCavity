...
#pragma once
#include "Simulator.h"
#include "Particle_2d_i_cst.h"

namespace SIM {

	template <typename real, enum Dim dim>
	class Mls_2d_i_cst : public Simulator < real, dim, Mls_2d_i_cst<real, dim> > {
...
#pragma omp parallel for
			...
			for (int p = 0; p < int(part->np); p++) {
				if (part->type[p] != FLUID) continue;
				part->pos[p] += (1. / 12.)* para.dt * (5.*part->vel2[p] + 8.*part->vel1[p] - 1.* part->vel_m1[p]);
			}
		}
...
#pragma omp parallel for
		...
		void init_() {
			part = new Particle_2d_i_cst<real, dim>();
			part->clean();
			*part << "Geo.in";
	...
	public:
		Particle_2d_i_cst<real, dim>*  part;
...
#pragma omp parallel for
		...
		void makeLhs_p() {
			...
			for (unsigned p = 0; p < part->np; p++) {
				...
				auto mm = part->invMat[p];
				if (part->type[p] == BD1) {
					mm = part->bd2InvMat.at(p);
				}
...
#pragma omp parallel for
			...
			for (int p = 0; p < int(part->np); p++) {
				...
				if (part->type[p] == BD1) {
					const auto _inner = part->bdnorm.at(p);
					auto inner = vecp();
					inner << _inner.x, _inner.y, _inner.z, 0., 0.;
					const auto a = part->bd2InvMat.at(p)* inner;
					const auto cst = part->neumann.at(p)*part->w3(0.)* (part->varrho[0] * part->poly_lap_0).dot(a);
					mSol->b[p] -= cst;
				}
			}
		}
