...
#pragma once
#include "Simulator.h"
#include "Particle_2d_j.h"

namespace SIM {

	template <typename real, enum Dim dim>
	class Mls_2d_j_euler : public Simulator < real, dim, Mls_2d_j_euler<real, dim> > {
...
#pragma omp parallel for
			...
			for (int p = 0; p < int(part->np); p++) {
				if (part->type[p] != FLUID) continue;
				part->pos[p] += para.dt * (part->vel1[p]);
			}
		}
...
#pragma omp parallel for
		...
		void init_() {
			part = new Particle_2d_j<real, dim>();
			part->clean();
			*part << "Geo.in";
			...
			part->init2d_x();
			calInvMat();
			pos_tmp = std::vector<vec>(part->pos);
		}

	public:
		Particle_2d_j<real, dim>*  part;
