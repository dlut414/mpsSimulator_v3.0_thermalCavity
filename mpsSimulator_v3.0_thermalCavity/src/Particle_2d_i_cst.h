...
namespace SIM {
	...
	class Particle_2d_i_cst : public Particle< real, dim, Particle_2d_i_cst<real, dim> > {
...
#endif
					auto mm_ = mm.block<2, 2>(0, 0);
					if (abs(mm_.determinant()) < eps_mat) invMat[p] = matpp::Zero();
					else invMat[p].block<2, 2>(0, 0) = mm_.inverse();
				}
				else invMat[p] = mm.inverse();

				if (type[p] == BD1) {
					const auto _inner = bdnorm.at(p);
					auto inner = vec13();
					inner << _inner.x, _inner.y, _inner.z;
					matpp nn = matpp::Zero();
					nn.block<1, 3>(0, 0) += w3(0.) * inner[0] * inner;
					nn.block<1, 3>(1, 0) += w3(0.) * inner[1] * inner;
					nn.block<1, 3>(2, 0) += w3(0.) * inner[2] * inner;
					auto bdmm = mm + nn;
					auto& inv = bd2InvMat.at(p);
					if (abs(bdmm.determinant()) < eps_mat) {
#if DEBUG
						std::cout << mm.determinant() << std::endl;
#endif
						auto bdmm_ = bdmm.block<2, 2>(0, 0);
						if (abs(bdmm_.determinant()) < eps_mat) inv = matpp::Zero();
						else inv.block<2, 2>(0, 0) = bdmm_.inverse();
					}
					else inv = bdmm.inverse();
				}
			}
		}
...
#pragma omp parallel for
	...
	public:
		...
		std::map<unsigned, matpp> bd2InvMat;
