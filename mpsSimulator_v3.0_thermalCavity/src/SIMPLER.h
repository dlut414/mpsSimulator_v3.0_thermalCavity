...
namespace SIM {
	...
	class SIMPLER : public Simulator <R, D, SIMPLER<R, D, P>> {
...
#pragma omp parallel for
			...
			for (int p = 0; p < int(part->np); p++) {
				...
				const MatDD nablaU = part->grad(part->vel1, p);
				const Vec conv = (part->vel1[p].transpose()* nablaU).transpose();
				const Vec rhs = (conv - g) / para.Pr;
				for (auto d = 0; d < D; d++) {
		...
		__forceinline void makeLhs_v_2() {
			coef.clear();
			for (unsigned p = 0; p < part->np; p++) {
				if (part->type[p] == BD1 || part->type[p] == BD2) {
					for (auto d = 0; d < D; d++) {
		...
		__forceinline void makeRhs_v_2() {
#if OMP
#pragma omp parallel for
#endif
			for (int p = 0; p < int(part->np); p++) {
				...
				Vec g = Vec::Zero(); g[1] = para.Ra* part->temp[p];
				const Vec rhs = (-part->grad(part->phi, p) + g) / para.Pr;
				for (auto d = 0; d < D; d++) {
					mSol->rhs[D*p + d] = rhs[d];
				}
			}
